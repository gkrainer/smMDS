...
def get_ptufilename():
    """
    Open a Tk interface for searching .ptu file.
    """
    root = tk.Tk()
    root.withdraw()
...
def get_bursts(ptufilename, user_setting=None):
    """
    get_bursts(ptufilename, user_setting)
    return photons_dataframe, bursts_dataframe

    argument
        ptufilename is a link to a .ptu file
        user_setting is a dictionary of different setting user can add
        by default user_setting is as following
        user_setting = {"set_lee_filter": 4,
                        "threshold_iT_signal": 0.05,
                        "threshold_iT_noise": 0.05,
                        "min_phs_burst" : 5,
                        "min_phs_noise" : 100,
                        "filter_name" : "addLeefilter",
                        "show_plot" : False}
        "set_lee_filter" is a parameter on which value the filter should
        applied for the interphoton time.
        "threshold_iT_signal" is the max inter-photon time (iT) signal in ms
        "threshold_iT_noise" is min inter-photon time (iT) noise in ms
        "min_phs_burst" is minimum number of photons per burst
        "min_phs_noise" is minimum number of photons per noise region
        "filter_name" can be
                            moving avergae of matlab "matlabLeefilter"
                            additif Lee filter "addLeefilter"
        "show_plot" decide to display or not the plot generated
    return
            photons_dataframe which is 2D arrays containing 2 column
                the first column is the channel number
                the second column is the time of photon detection
            bursts_dataframe which is 2D arrays containing 3 column
                the first column is the time when burst start
                the second column is the intensity associate
                the third column is the burst duration

    TODO --> Add a boolean condition in user setting to know if we want or not
    display the photons dataframe
    """
    # Load default setting
    default_setting = {"set_lee_filter": 4,
                       "threshold_iT_signal": 0.05,
                       "threshold_iT_noise": 0.05,
                       "min_phs_burst" : 5,
                       "min_phs_noise" : 100,
    ...
    # Transform anything in dict user_setting into the needed parameters
    ...
    boolShow = user_setting["show_plot"]
    # Find the name of output file remove "ptu" replace by "out"
    outputfile_name = ptufilename[:-3] + "out"
    # if the outputfile_name does not exist
    # transform the binary data ".ptu" in readable data ".out" using readPTU
    if not os.path.isfile(outputfile_name):
        readPTU(ptufilename, outputfile_name)
    ...
    Photons[:, 1] = Photons[:, 1] / 1000  # time transform in (ns) : (ps/1000)

    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    # %% Find burst
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #rename the variable channel and time when photon detect
    macroT = Photons[:, 1]  # (ns)
    channel = Photons[:, 0]

    # find inter-photon time and filter the result
    interPhT = np.diff(macroT)  # (ns)
    if user_setting["filter_name"] == "addLeefilter":
        interLee = leeFilter1D_Add(interPhT, setLeeFilter)
    elif user_setting["filter_name"] == "matlabLeefilter":
        interLee = leeFilter1D_matlab(interPhT, setLeeFilter)
    else:
        raise RuntimeError('filter name does not exist')

    # 1st filter
    # threshold for inter photon time
    # indexSig is for molecule analysis, indexSigN is for the noise analysis
    indexSig = np.asarray(np.where(np.logical_and(0.4 < interLee,
                                                  interLee < (threIT * 1e6))),
                          dtype=int)
    indexSigN = np.asarray(np.where(interLee > (threIT2 * 1e6)))

    # if indexSig and indexSigN are not empty process the second filter
    # todo distingusih between indexSigN and indexSig
    # ->if no indexSigN still, the code still count Number of molecule.
    if np.asarray(indexSig).size != 0:
        bStart, bLength = burstLoc(indexSig, 1)
        bStartN, bLengthN = burstLoc(indexSigN, 1)

        # 2nd filter
        # minimum photons per burst minimum photons for noise
        ...
        NI = np.zeros(np.size(bStartLong))  # intensity for each burst
        ...
        for i in range(0, np.size(bStartLong)):
            ...
            # TBurst is duration for each burst
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    #      #% show burst duration histogram
        ...
        d1 = {'channel': channel, 'photons': Photons[:,1]}
        df1 = pd.DataFrame(data=d1)
        d2 = {'Burst timestart': Photons[bStartLong, 1],
              'Burst intensity': NI,
