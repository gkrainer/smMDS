...
def readPTU(inputfile_name, outputfile_name):
    ...
    outputfile = open(outputfile_name, "bw")
    ...
    def gotPhoton(timeTag, channel, dtime):
        ...
        if isT2:
            macrotime_Photon = timeTag * globRes * 1e12
            outputfile.write(struct.pack("d",channel))
            outputfile.write(struct.pack("d",macrotime_Photon))
        else:
            raise RuntimeError("not implemented for T3")
